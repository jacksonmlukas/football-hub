...
| board build fails on a source | SoS, touchdown luck and the two league checks are advisory: the board is thinner and still correct. **Durability is not** — it feeds Corrected ADP, so without it THE PICK ranks on a different order (issue #121). Touchdown luck was in that sentence until 2026-09-10, when #186/#48 withdrew its coefficients; the column is still shown and now moves no ranking | it will say which, print `built without: ...`, and print `CORRECTED ADP is missing durability` when the ranking is the affected one; the board still builds |
| `market corrections FAILED (...)` then `BUILD FAILED` | a defect, not an outage. That stage computes the blended projection, both corrections and Corrected ADP — what THE PICK ranks on — so it is the one stage that does **not** degrade: dropping it would leave a board ranking on raw consensus while claiming it fell back on purpose. An ESPN outage never lands here; it prints `ESPN ADP unavailable` and the stage is skipped | you get the last good board, below. Fix the defect and rebuild — the named stage is where to look |
...
| `CORRECTIONS APPLIED whose coefficients are disputed` | not a failure. The durability correction applies two numbers a 2026-09-07 refit disagrees with — `BETA['WR']` about half the size the data supports, `INJURY_BETA['OUT']` about twice it on 68 observations — and #48 says the board names what it applied rather than applying it silently | nothing on the night. The numbers still apply on purpose: withdrawing a correction mid-season is itself an unmeasured change. docs/fitted-corrections.md has the intervals |
