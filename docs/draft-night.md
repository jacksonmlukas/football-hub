...
| board build fails on a source | SoS, touchdown luck, durability and the two league checks each degrade on their own — they are advisory, and the board is correct without them | it will say which, and print `built without: ...`; the board still builds |
