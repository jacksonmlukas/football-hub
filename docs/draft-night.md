...
| board build fails on a source | SoS and the two league checks are advisory: the board is thinner and still correct. **Touchdown luck and durability are not** — they feed Corrected ADP, so without them THE PICK ranks on a different order (issue #121) | it will say which, print `built without: ...`, and print `CORRECTED ADP is missing ...` when the ranking is one of the affected ones; the board still builds |
