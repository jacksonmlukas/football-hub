...
| the digest | `config_digest` `b1f69382` → `ce7c7f8b`: `quarterback.ELO_PER_POINT` left its view (the harness's conversion now, read by no prediction) and `STALE_AFTER_DAYS` is re-keyed under `schedule` at the same value; the pin paragraph in `tests/unit/test_config.py` records both |
