...
| [0002](adr/0002-one-forecaster-protocol.md) | One `Forecaster` protocol, league as a field | accepted, **payoff under question** | a head-to-head comparison the protocol carries — **moved**, the protocol has one adapter and the schema has two (**#136**) |
...
| [0009](adr/0009-championship-equity-does-not-pick.md) | Championship equity does not pick | accepted | −19.66 pts/team-game, CI [−23.16, −16.20], n=80, lost all four seasons; re-measured on the fixed bracket at **−17.30**, verdict unchanged. **Silent on where the removed arm lives** (**#198**) |
