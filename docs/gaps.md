...
|---|---|---|---|
| `data/processed/roster.parquet` | `hub/season/roster.py` | **RESOLVED 2026-09-04** | Written by `hub.season.roster`, which joins your ESPN team to the board and takes `predict.moments` over `proj_blend`. It was blocked on the draft, not on design: the roster was unknowable until Sep 3. `hub.season.lineup` now runs end to end for the first time. K and D/ST carry `projected = False` rather than `mu = 0.0`, because the board's scope is `DRAFTED_POSITIONS`. |
