...
def grid_from_schedule(season: int, cache: Path | None = None, *,
    ...
    `hub.models.ratings.rated_games`'s -- `hub.schedule`'s number, the betting market's on
    every row -- so they cannot disagree about which number that is either. They did, for a
    day: the weekly prediction moved onto the dated snapshots and this was left reading
    nflverse's own field, which upstream leaves empty for the late season. That planned
    twelve of eighteen weeks and reported the rest unpriced while the store held every game
    of the season, week 18 included. From #218 to #299 the seam quarterback-adjusted the
    rows the staleness field marked as having no live price; #299 pulled that, because the
    source names a new starter only after his first game and the adjustment could only ever
    move a line that had already priced him. The survivor horizon is where a timely starter
    source would matter most -- a starter ruled out in October reaches a week-14 pick
    through nothing else -- and `docs/qb-adjustment.md` names what would let one back.
    ...
    Each row also carries the game's `close_spread` (the home side's, as priced) and, since
    #281, `price_source` -- `hub.schedule.priced_games`'s three-way label (`live`, `stale`,
    `schedule`) -- so the grid says which source priced each row. The two adjustment
    columns it carried from #218 to #299, `qb_adjustment` and `adjusted_by`, left with the
    adjustment: no pick is moved, so no row has anything to say about it.
    """
    from hub.models import ratings
    from hub.models.market import MARGIN_SD, normal_cdf

    games = ratings.rated_games(season, at=at, cache=cache, base=base)
    rows = []
    for r in games.filter(pl.col("close_spread").is_not_null()).iter_rows(named=True):
        ...
        gid = r["game_id"]
        rated = (float(r["close_spread"]), source)
    ...
    return pl.DataFrame({"week": [r[0] for r in rows], "team": [r[1] for r in rows],
                         ...
                         "price_source": [r[8] for r in rows]},
                        schema={"week": pl.Int64, "team": pl.Utf8, "win_prob": pl.Float64,
                                ...
                                "close_spread": pl.Float64, "price_source": pl.Utf8})
