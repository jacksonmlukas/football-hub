...
def test_a_spent_team_stays_spent_across_a_second_publish(site, base, monkeypatch):
    """Review 2026-09-12, reproduced: the first publish after a played week trims that
    week's row out of the artifact, and `spent_teams` read history only off the rows -- so
    the *second* publish of the same week saw no played rows, forgot KC, and offered him
    again. The envelope's own `spent` list is the ledger and is carried forward."""
    import hub.season.survivor as sv
    site.mkdir(parents=True, exist_ok=True)
    (site / "survivor.json").write_text(json.dumps(
        {"name": "survivor", "n": 3, "rows": [{"week": 1, "team": "KC", "win_prob": 0.9},
    ...
    assert second["spent"] == ["KC"], "the second slate of the week forgot what the first knew"
...
def test_a_survivor_plan_from_another_season_spends_nothing(site, base, monkeypatch):
    ...
    site.mkdir(parents=True, exist_ok=True)
    (site / "survivor.json").write_text(json.dumps(
        {"name": "survivor", "season": 2025, "n": 1, "spent": ["SF"],
