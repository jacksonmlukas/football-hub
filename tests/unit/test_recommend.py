...
from hub.draft.board import Board, recommend


def _board(n=60):
    """A frame; `recommend` takes the Board, which `_served` makes of it (#295)."""
    return pl.DataFrame({
...
def test_an_ecr_only_board_is_not_given_a_draft_market_it_does_not_have():
    ...
    _, out = recommend(_served(_board().drop("adp")), current_pick=3, top=5)
...
def test_first_pick_is_scarcity_mode():
    mode, _ = recommend(_served(_board()), current_pick=3)
...
def test_second_round_pick_is_value_mode():
    ...
    mode, _ = recommend(_served(_board()), current_pick=22)
    assert mode == "value"


def test_value_mode_ranks_by_raw_vor():
    _, out = recommend(_served(_board()), current_pick=22, top=3)
...
def test_scarcity_mode_ranks_by_cost_of_waiting():
    _, out = recommend(_served(_board()), current_pick=3, top=5)
