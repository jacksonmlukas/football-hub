...
#
# The excision harness below needs, for each guard, an input that *only* that guard catches.
# Two of the six had none: a committed `.env` and a data file staged but not yet committed.
# Without them, deleting the guard leaves another one covering the same input and the
# excision reports a load-bearing guard that is not.
...
def test_a_committed_env_file_blocks_the_flip(repo):
    ...
    which is what makes it the input only the .env guard catches."""
...
# it is not the property that matters: a guard that has been quietly weakened -- a pattern
# that stops matching, a check whose two halves are never tied together -- still passes it,
# because some *other* guard catches the planted input, or because the input was never
# reaching the guard at all. This file has failed exactly that way twice.
#
# The stronger property, the one tests/contracts/test_guards_are_load_bearing.py already
# holds the Python source to: **delete the guard and the planted input must stop being
# caught.** One convention, written in shell comments:
#
#     # GUARD name [unit/test_preflight.py::test_x unit/test_preflight.py::test_y]: why.
#     ...
#     # /GUARD
#
# The grammar, the record type, the excision and the reading of a child run are all
# `tests/guardlib.py`, shared with the Python harness and the page's -- three near-identical
# copies until issue #65. The bracket resolves by guardlib's one rule: pytest selectors
# relative to `tests/`. It held a `-k` expression here until then, which is the same syntax
# as the Python harness's paths carrying a different meaning, and node ids are the stricter
# form besides -- a `-k` expression that stops matching quietly selects nothing, where a node
# id that stops resolving is an error naming the test that went missing.
#
# The selectors are written rather than derived because there is no filename convention to
# derive them from in shell: one script, one test module, six guards. That is a second thing
# to keep in step, so `test_every_declared_block_names_tests_that_exist` reads them and the
# control run below fails loudly when one selects nothing.
#
...
#      cut passed its mutant tree through PYTHONPATH, which pyproject's `pythonpath` shadows,
#      so every mutant ran against the real source and all four guards reported green. Here
#      the equivalent proof is `test_the_child_run_uses_the_script_it_is_handed`.
...
# declared instead:
#
#     # UNPROVED name [the test that expires the exception]: why it cannot be proved here.
#     ...
#     # /UNPROVED
#
# Deliberately the `# GUARD` block shape with a different verb, built from the same
# `guardlib.marker`, rather than a fourth marker grammar. The bracket keeps its one meaning:
# for a `# GUARD` the selectors name the tests that must go red when the block is deleted,
# and for an `# UNPROVED` they name the test that fails the day the exemption expires. Both
# answer "which test settles this", which is what the bracket has meant since issue #65.
...
def _declared_guards() -> list[Guard]:
    return declared(SCRIPT.read_text(), SHELL_GUARD, SCRIPT)
...
def _unproved(text: str) -> dict[str, str]:
    ...
    return {g.name: text[g.span[0]:g.span[1]] for g in declared(text, SHELL_UNPROVED)}


def _unmarked_guards(text: str) -> list[str]:
    ...
    refusing line in and watch this name it. Plain substring, not `\\bfail=1\\b`:
    `canary_fail=1` and any other suffixed flag counts too, because over-reporting here costs
    a comment and under-reporting is the whole defect.

    Whole-comment lines are the one exclusion, and they are excluded because a guard cannot
    live in one: the prose explaining this convention says `fail=1` several times in both
    files, and reading those as unmarked guards is the fires-on-its-own-documentation shape
    the pattern charsets above were tightened to avoid. A real refusing line carrying a
    trailing comment is untouched -- only a `#` before any other content on the line makes it
    a comment here.
    """
    covered: set[int] = set()
    for pattern in (SHELL_GUARD, SHELL_UNPROVED):
...
def _child(script: Path, selectors: tuple[str, ...]) -> subprocess.CompletedProcess:
    """Run the tests a guard names against the script at `script`."""
    env = os.environ.copy()
...
def test_the_gate_declares_the_guards_it_makes():
    ...
    spent on the gitleaks call -- so it read as "one unmarked guard is fine" and could not
    tell which one, nor notice the spender changing. Every unmarked guard is reported here
    by line and by source text instead."""
    found = _declared_guards()
    assert len(found) >= 5, f"only found {[g.name for g in found]}"
    unmarked = _unmarked_guards(SCRIPT.read_text())
    assert unmarked == [], (
        "these refusing lines sit inside no # GUARD and no # UNPROVED block, so nothing "
        "proves they fire:\n  " + "\n  ".join(unmarked) + "\n"
        "Wrap each in `# GUARD name [unit/test_preflight.py::test_x]: why` and let the "
        "excision harness below prove it, or -- only if it genuinely cannot be proved here "
        "-- declare it as an `# UNPROVED` exception and add its name to UNPROVED_HERE.")


@pytest.mark.skipif(MUTANT_RUN, reason="a child run of the excision harness")
def test_every_declared_block_names_tests_that_exist():
    """The selectors are the one thing here that is written rather than derived, so this is
    the check the Python harness gets for free from its filename convention. A selector that
    has gone stale makes the control run below fail with pytest's own usage error, which is
    late and unclear; naming it here is early and clear."""
    text = SCRIPT.read_text()
    blocks = declared(text, SHELL_GUARD, SCRIPT) + declared(text, SHELL_UNPROVED, SCRIPT)
    stale = [(g.name, s) for g in blocks for s in (g.selectors or ("",))
...
def test_a_second_unmarked_refusing_line_is_named_immediately():
    ...
    got = _unmarked_guards(spliced)
    assert len(got) == 1, f"the splice was not the only unmarked line: {got}"
    assert "grep -q something_new" in got[0], (
        f"the report does not say what the unmarked line is: {got[0]}")
    assert got[0].startswith("line "), f"nor where it is: {got[0]}"


@pytest.mark.skipif(MUTANT_RUN, reason="a child run of the excision harness")
def test_a_refusing_line_carrying_a_trailing_comment_is_still_unmarked():
    """The other side of that exclusion. Whole-comment lines are skipped so the prose about
    this convention is not read as a guard; a real refusing line that happens to end in a
    comment must not leave through the same door."""
...
@pytest.mark.skipif(MUTANT_RUN, reason="a child run of the excision harness")
def test_the_unproved_guard_is_named_rather_than_counted():
    """The exception is one specific guard, identified by what it is. A second one is a new
    unproved guard and has to be argued for here, in this set, rather than inherited from a
...
def test_the_exception_is_recorded_where_someone_meets_it():
    """The reason has to sit on the guard, not only in this file: the person who adds the
    eighth guard is reading the script, and `# UNPROVED` two lines above the call is what
    ...
    assert "fail=1" in block, "the exception does not wrap the guard it exempts"
...
def test_the_exception_expires_when_the_scanner_is_installed():
    ...
    prove nothing. Install the tool and that premise is gone -- the guard becomes provable,
    ...
    assert "third-party-secret-scanner" not in _unproved(SCRIPT.read_text()), (
        "gitleaks is on PATH, so the reason this guard is exempt from the excision harness "
        "no longer holds. Plant an input only gitleaks catches, wrap the call in a "
        "`# GUARD third-party-secret-scanner [unit/test_preflight.py::that_test]`, and drop "
        "the `# UNPROVED` block and its name from UNPROVED_HERE.")
...
def test_the_child_run_uses_the_script_it_is_handed(tmp_path):
    ...
    assert got.is_evidence, (
        ...
        + got.why_not_evidence() + "\n" + got.stdout[-2000:])


@pytest.mark.skipif(MUTANT_RUN, reason="a child run of the excision harness")
@pytest.mark.parametrize("guard", _declared_guards(), ids=repr)
def test_removing_the_guard_stops_the_planted_input_being_caught(guard, tmp_path):
    """Delete one guard, run the tests it names, require red.

    Three outcomes have to be told apart, and `guardlib.outcome` is what tells them apart in
    all three harnesses. A child that reports a test *failure* means the guard is
    load-bearing. A green one means the planted input is caught by something else, or by
    nothing -- either way nothing proves this guard fires. A mutant that will not parse, a
    control run that is not already green, or a child that goes red without any test failing,
    is a harness error and is reported as one.
    """
    mutant_text = _without(guard)
    ...
    if parses.returncode != 0:                          # trap 1
        pytest.fail(
            f"removing guard {guard.name!r} leaves a script bash will not parse "
            f"({parses.stderr.strip()}). Widen the markers so the block stands alone -- as "
            f"written, a red run below would prove nothing.")

    control = _child(SCRIPT, guard.selectors)
    assert control.returncode == 0, (
        f"the control run for {guard.name!r} is not green before anything is removed, so a "
        f"red mutant run would prove nothing. Its selectors "
        f"({' '.join(guard.selectors)}) "
        f"{'do not resolve to tests that exist' if control.returncode == 4 else 'selected failing tests'}:\n"
        + control.stdout[-2000:])
    ...
    # and a red run for that reason proves nothing either. An actual test failure is the only
    # evidence, and it is read off pytest's short summary rather than matched for substrings.
    got = outcome(_child(mutant, guard.selectors))
    if got.errors and not got.failed:                   # pragma: no cover - harness error
        pytest.fail(
            ...
            + got.stdout[-2000:])
    assert got.is_evidence, (
        f"{guard.name!r} was deleted and its tests still pass, so nothing proves it fires. "
